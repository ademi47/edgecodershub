...
export default function BookingSystem() {
    const isAdminRoute = URLsearchParams.get('admin') === 'true';
            ...
            if (bookingsData.booking && Array.isArray(bookingsData.booking)) {
                ...
                const transformedBookings = bookingsData.booking.map((item: any) => {
                    ...
                    return {
                        ...
                        phone: item.json.Phone?.toString() || '',
                        personType: item.json.PersonType || '',
                        reason: item.json.Reason || '',
                        contactMethod: item.json.ContactMethod || '',
                        date: item.json.Date || '',
                ...
                console.log('📊 Accepted bookings:', transformedBookings.filter(b => b.status === 'accepted'));
    ...
    const isSlotAvailable = (date: Date, time: string) => {
        ...
        console.log(`${isBooked ? '❌ BOOKED' : '✅ AVAILABLE'}`);
        return !isBooked;
    };
    ...
    const handleAdminLogin = () => {
        if (adminPassword === '@dmin@Edg3Cod3r$') {
